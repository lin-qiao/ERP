	...
	<view>
		...
		<mescroll-body  top="90" ref="mescrollRef" @init="mescrollInit"  @down="downCallback" :up="upOption" @up="upCallback">
			<scroll-view class="list" scroll-y>
				<navigator class="item"  hover-class="none" v-for="item in data" :key="item.id" :url="'/pages/purchaseDetail/purchaseDetail?purchaseSn=' + item.purchaseSn">
					<view class="left">
						<view class="sn">{{item.purchaseSn}}</view>
						<view class="time">{{dataFormat(item.createTime, 'yyyy-MM-dd hh:mm:ss')}}</view>
					</view>
					<view class="right">
						<view class="num">采购:{{item.totalNumber}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData(){
				uni.showLoading()
				purchaseList(this.params).then(({data, total}) => {
					this.total = total;
