...
const findAndCountAll = async (page, size, goodSn, brandId, uid, order) => {
	...
	return goodsModel.findAndCountAll({
		...
		attributes: [
			...
			[Sequelize.literal('`stock`.`cost_price` * `stock`.`number`'), 'totalCostPrice'],
 ...
 * @return 
 */
const create = async function(goodsId, sizeId, number, costPrice, totalPrice, uid) {
...
const destroyByGoodsId = async function(goodsId, uid) {
