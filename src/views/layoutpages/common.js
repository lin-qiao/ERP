...
export const maxHeight = (pagination, queryForm, toolBar, ve_max_height, other) => {
  onresize = () => {
  ...
  let paginationHeight = pagination.value ? pagination.value.$el.offsetHeight || 0 : 0
  let queryFormHeight = queryForm.value ? queryForm.value.$el.offsetHeight || 0 : 0
  let otherHeight = other && other.value ? other.value.offsetHeight || 0 : 0
  ve_max_height.value =
    docHeight - paginationHeight - queryFormHeight - toolBarHeight - otherHeight - 130
}
/**
...
export const getAsyncRouteName = async (title, path, name, { router, route }) => {
  const AsyncComponent = defineAsyncComponent(() => import('@/views/layoutpages/' + path + '.vue'))
  const app = {
    components: {
      FunctionPage,
      AsyncComponent
