<template>
	...
	<el-form ref="queryForm" :inline="true" :model="params">
		<el-form-item label="商品：" prop="goodSn">
			<el-select v-model="goodsInfo" value-key="id" filterable remote clearable reserve-keyword
				placeholder="请输入商品名称/货号" :remote-method="getGoodsList" :loading="loading" @change="changeGoodsInfo">
				<el-option v-for="item in goodsList" :key="item.articleNumber" :label="`${item.name} / ${item.goodSn}`"
					:value="item">
		...
		<el-form-item label="尺码：" prop="sizeId">
			<el-select clearable v-model="sizeId" placeholder="请选择尺码" @change="onSubmit(params, getDataList)">
				<el-option v-for="item in sizeList" :key="item.id" :label="item.sizeName" :value="item.id">
				</el-option>
			</el-select>
		</el-form-item>
		<el-form-item label="日期：" prop="createTime">
			<el-date-picker clearable v-model="createTime" type="daterange" value-format="YYYY-MM-DD" unlink-panels
	...
	<!-- 列表 -->
	<table-com class="table" :columns="tableColumn" :data="tableData" style="width: 100%" :max-height="ve_max_height">
	</table-com>
	<!-- 分页 -->
	<el-pagination ref="pagination" background @size-change="val => handleSizeChange(val, params, getDataList)"
		@current-change="val => handleCurrentChange(val, params, getDataList)"
		:hide-on-single-page="total <= size ? true : false" layout="total, sizes, prev, pager, next, jumper"
		:page-sizes="[10, 20, 50, 100]" :current-page="page" :page-size="size" :total="total">
	</el-pagination>
</template>
