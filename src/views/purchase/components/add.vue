<template>
	<el-card>
		...
		<el-form ref="queryForm" :inline="true">
			<el-form-item label="供应商" props="supplierId">
				<el-select v-model="supplierInfo" value-key="id" placeholder="请选择供应商">
			...
			<el-table :data="tableData" show-summary :summary-method="getSummaries">
				...
				<el-table-column prop="goodsName" label="商品" align="center" min-width="200">
					<template v-slot="{ row, $index }">
						<el-select style="width: 100%;" v-model="row.goodsInfo" value-key="goodSn" filterable remote
							reserve-keyword placeholder="请输入商品名称/货号" :remote-method="getGoodsList" :loading="loading"
							@change="changeGoods($index)">
							<el-option v-for="item in goodsList" :key="item.goodSn" :label="item.name" :value="item">
								{{item.name}} / {{item.goodSn}}
							</el-option>
						</el-select>
					</template>
				</el-table-column>
				<el-table-column prop="goodsSn" label="货号" width="150" align="center">
				</el-table-column>
				<el-table-column prop="sizeId" label="尺码" width="120" align="center">
					<template v-slot="{ row, $index }">
						<el-select style="width: 100%;" v-model="row.sizeId" @change="changeSize($index)">
							<el-option v-for="item in row.sizeList" :key="item.id" :label="item.sizeName"
								:value="item.id">
							</el-option>
						</el-select>
					</template>
				...
				<el-table-column prop="price" label="单价" width="120" align="center">
					<template v-slot="{ row }">
						<el-input-number style="width: 100px;" v-model="row.price" :precision="2" :controls="false"
							:min="0">
						</el-input-number>
					</template>
				</el-table-column>
	...
	<select-size v-if="visible" v-model:visible="visible" :goodsId="goodsId" type="add" @confirm="handleConfirmSize">
	</select-size>
</template>
	...
	export default {
		...
		setup(props, {
			...
			const changeSize = (index) => {
				const obj = tableData.value[index];
				const hasSize = tableData.value.filter(item => item.goodsId == obj.goodsId && item.sizeId == obj
					.sizeId);
				if (hasSize.length >= 2) {
					proxy.$confirm("已经存在相同颜色尺码的商品，是否合并数量为一条记录？", "提示", {
						...
						.then(() => {
							tableData.value.forEach((item, i) => {
								if (item.goodsId == obj.goodsId && item.sizeId == obj.sizeId && i !=
									index) {
			...
			const handleConfirmSize = (arr) => {
				...
				arr.forEach(item => {
					// 是否已经选择过相同的尺码
					const hasSize = tableData.value.filter(obj => obj.goodsId == goodsInfo.value.id && obj
						.sizeId == item.sizeId);
					if (hasSize.length) {
			...
			const onSubmit = async () => {
				...
				const {
					...
					data,
					message
				} = await VE_API.purchase.purchaseAdd({
