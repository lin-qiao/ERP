...
const stockGoodsList = async function (ctx) {
  ...
  const uid = ctx.session.user_id;
  try {
    const data = await stockGoodsModel.findAndCountAllByGoods(
      parseInt(page),
      parseInt(size),
      goodSn,
      brandId,
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const stockInfo = await stockModel.findByGoodsId(obj.goodsId, uid);
      obj.stockInfos = util.filterUnderLine(stockInfo);
    }
