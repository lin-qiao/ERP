...
const saleCount = async function(ctx) {
	...
	try {
		//总数量
		const saleNumber = await businessFlowModel.sumNumber(3, uid, 'number', createTimeBegin, createTimeEnd) || 0;
		const saleReturnNumber = await businessFlowModel.sumNumber(4, uid, 'number', createTimeBegin, createTimeEnd) || 0;
		const numberTotal =  + saleNumber + saleReturnNumber;
		//总销售额
		const salePrice = await businessFlowModel.sumNumber(3, uid, 'totalBusinessPrice', createTimeBegin, createTimeEnd) || 0;
		const saleReturnPrice = await businessFlowModel.sumNumber(4, uid, 'totalBusinessPrice', createTimeBegin, createTimeEnd) || 0;
		const priceTotal =  (salePrice * 100 - saleReturnPrice * 100) / 100;
		//总毛利
		const saleGrossProfit = await businessFlowModel.sumNumber(3, uid, 'grossProfitPrice', createTimeBegin, createTimeEnd) || 0;
		const saleReturnGrossProfit = await businessFlowModel.sumNumber(4, uid, 'grossProfitPrice', createTimeBegin, createTimeEnd) || 0;
...
const purchaseCount = async function(ctx) {
	...
	try {
		//总数量
		const saleNumber = await businessFlowModel.sumNumber(1, uid, 'number', createTimeBegin, createTimeEnd) || 0;
		const saleReturnNumber = await businessFlowModel.sumNumber(2, uid, 'number', createTimeBegin, createTimeEnd) || 0;
		const numberTotal =  + saleNumber + saleReturnNumber;

		//总成本
		const saleCostTotal = await businessFlowModel.sumNumber(1, uid, 'totalBusinessPrice', createTimeBegin, createTimeEnd) || 0;
		const saleReturnCostTotal = await businessFlowModel.sumNumber(2, uid, 'totalBusinessPrice', createTimeBegin, createTimeEnd) || 0;
